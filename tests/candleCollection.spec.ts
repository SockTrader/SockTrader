    ...
    it('Should fill all candle gaps until last interval occurrence before current time', () => {
        ...
        collection.on("update", (candles: ICandle[]) => {
            ...
            expect(testCandles).to.deep.equal([
                ...
                {open: 2,high: 2, low: 2,close: 2, volume: 0, timestamp: start.clone().add(4, "minutes").toISOString()},
                {open: 2,high: 2, low: 2,close: 2, volume: 0, timestamp: start.clone().add(3, "minutes").toISOString()},
                {open: 2,high: 2, low: 2,close: 2, volume: 0, timestamp: start.clone().add(2, "minutes").toISOString()},
                {open: 1.5,high: 3, low: 1,close: 2, volume: 5, timestamp: start.clone().add(1, "minutes").toISOString()},
