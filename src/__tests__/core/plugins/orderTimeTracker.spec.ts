import {IOrder, OrderStatus, ReportType} from "../../../sockTrader/core/types/order";
...
describe("TradeProfitCalculator", () => {
    ...
    test("Should not log when new order has been created", () => {
        timeTracker.onReport({id: "1", status: OrderStatus.NEW} as IOrder);
        expect(walletLogger.info).toHaveBeenCalledTimes(0);
    });

    test("Should log when an order has been filled", () => {
        timeTracker.onReport({id: "1", status: OrderStatus.FILLED} as IOrder);
    ...
    test("Should log when an order has been partially filled", () => {
        timeTracker.onReport({id: "1", status: OrderStatus.PARTIALLY_FILLED} as IOrder);
    ...
    test.each([
        ...
        timeTracker.onReport({id: "1", status: status} as IOrder);
    ...
    test("Should move replaced orders to new ID", () => {
        ...
        timeTracker.onReport({id: "2", originalId: "1", reportType: ReportType.REPLACED} as IOrder);
    ...
    test("Should log time of correct order when multiple orders are placed", () => {
        ...
        jest.spyOn(Date, "now").mockImplementation(() => 1479427200000 + (60 * 5) * 1000);
        timeTracker.onReport({id: "2", status: OrderStatus.NEW} as IOrder);
        ...
        timeTracker.onReport({id: "2", status: OrderStatus.FILLED} as IOrder);
