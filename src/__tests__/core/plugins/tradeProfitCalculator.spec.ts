...
describe("TradeProfitCalculator", () => {
    ...
    test("Should not log when selling too much assets", async () => {
        calculator.onReport({side:OrderSide.SELL, price: 50, quantity: 5, status: OrderStatus.FILLED} as Order);
        expect(walletLogger.info).toBeCalledTimes(0);
    });

    test("Should not log when status is not FILLED or PARTIALLY_FILLED", async () => {
        calculator.onReport({side:OrderSide.BUY, price: 50, quantity: 5, status: OrderStatus.NEW} as Order);
        expect(walletLogger.info).toBeCalledTimes(0);
    });

    test("Should log when status is FILLED", async () => {
        calculator.onReport({side:OrderSide.BUY, price: 50, quantity: 5, status: OrderStatus.FILLED} as Order);
    ...
    test("Should log profit/loss compared to average buy price", async () => {
        calculator.onReport({side:OrderSide.BUY, price: 50, quantity: 5, status: OrderStatus.FILLED} as Order);
        calculator.onReport({side:OrderSide.BUY, price: 150, quantity: 5, status: OrderStatus.FILLED} as Order);
        ...
        calculator.onReport({side:OrderSide.SELL, price: 200, quantity: 3, status: OrderStatus.FILLED} as Order);
        calculator.onReport({side:OrderSide.SELL, price: 400, quantity: 2, status: OrderStatus.FILLED} as Order);
        ...
        calculator.onReport({side:OrderSide.BUY, price: 300, quantity: 5, status: OrderStatus.FILLED} as Order);
        expect(walletLogger.info).toHaveBeenNthCalledWith(5,"Avg buy: 200");

        calculator.onReport({side:OrderSide.SELL, price: 100, quantity: 5, status: OrderStatus.FILLED} as Order);
