...
export default class HitBTC extends Exchange {
    ...
    public constructor(private pubKey: string = "", private secKey: string = "") {
        super();
    }

    public static getInstance(pKey: string = "", sKey: string = "") {
        if (!HitBTC.instance) {
     ...
     * @param qty
     */
    public adjustOrder(order: IOrder, price: number, qty: number): void {
     ...
     */
    public cancelOrder(order: IOrder): void {
     ...
     * @param privateKey
     */
    public login(publicKey: string, privateKey: string): void {
     ...
     * Update candles
     */
    public onUpdateCandles<K extends keyof CandleCollection>(pair: string, data: ICandle[], interval: ICandleInterval, method: Extract<K, "set" | "update">): void {
     ...
     */
    public onUpdateOrderbook<K extends keyof Orderbook>(response: IOrderbookData, method: Extract<K, "setOrders" | "addIncrement">): void {
    ...
    public subscribeCandles = (pair: string, interval: ICandleInterval): void => this.send("subscribeCandles", {
        symbol: pair,
        period: interval.code,
    });

    public subscribeOrderbook = (pair: string): void => this.send("subscribeOrderbook", {symbol: pair});

    public subscribeReports = (): void => this.send("subscribeReports");
