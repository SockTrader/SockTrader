...
export const getStartOfNextInterval = (date: Date, compression: number, timeframe: TimeFrame, offset: number = 1): Date => {
...
export const aggregateCandles = (compression: number, timeframe: TimeFrame, intervalInSeconds?: number): OperatorFunction<Candle, Candle> => {
  return (source: Observable<Candle>): Observable<Candle> => {
    ...
    return new Observable<Candle>(subscriber => {
      source.subscribe({
        next(c) {
          if (firstCandle && candleInterval == null) { // automatically determine candle interval
